...
function Slider() {
  ...
  //       `${process.env.REACT_APP_SERVER_URL}/flights/search`,
  //       { source, destination, date }
  //     );
  
  //     if (res.data.status === "success") {
  //       const flights = res.data.flights;
  //       // 👉 Pass flight data via navigation or global state
  //       navigate({
  //         pathname: "/selectbus", // rename to `/flights` if needed
  //         search: `?from=${source}&to=${destination}&date=${date}`,
  //       }, { state: { flights } }); // ← pass flights using router state
  //     } else {
  //       setsource("");
  //       setdestination("");
  ...
  async function handleclicked() {
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_SERVER_URL}/flights/seed`, {
        source,
        destination,
        date,
      });
      ...
      if (res.data.status === "success") {
        ...
        error("Something went wrong while generating flights");
      }
    } catch (err) {
      console.log(err);
